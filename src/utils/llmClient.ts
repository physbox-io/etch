 ...
 * direct call is what runs. The same code therefore works in both, which is how
 * Mesh does it.
 */
...
export type LLMResult = {
   ...
   * to report success rather than apply half a document.
   */
  truncated: boolean;
};
...
async function callClaude(system: string, user: string, model: string): Promise<LLMResult> {
  ...
  if (!key) throw new LLMError('Add your Anthropic API key below to use this model.');
...
async function callGemini(system: string, user: string, model: string): Promise<LLMResult> {
  ...
  if (!key) throw new LLMError('Add your Google Gemini API key below to use this model.');
...
/**
 * Pulls the reply's JSON payload out of the surrounding prose.
 *
 * Models put the block in a fence, sometimes with commentary either side and
 * sometimes with the language tag missing, so this scans every fenced block and
...
/** The reply with its fenced blocks stripped — what's left is for the user. */
